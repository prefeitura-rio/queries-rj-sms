...
CASE
    WHEN LENGTH(TRIM({{ cns_column }})) != 15
        THEN FALSE

    -- Validation for CNS starting with 1 or 2
    WHEN SAFE_CAST(SUBSTR({{ cns_column }}, 1, 1) AS INT64) IN (1, 2)
        THEN (
            SELECT
                cns = CASE
                    WHEN 11 - resto = 10
                        THEN pis || '001' || CAST(dv AS STRING)
                    ...
                    *,
                    CASE
                        ...
                        WHEN 11 - resto = 10
                            THEN (
                                11 - MOD(
                                    d1 * 15 + d2 * 14 + d3 * 13 + d4 * 12 + d5 * 11 + d6 * 10
                                    + d7 * 9 + d8 * 8 + d9 * 7 + d10 * 6 + d11 * 5 + 2,
                                    11
                                )
                            )
                        ELSE 11 - resto
    ...
    WHEN SAFE_CAST(SUBSTR({{ cns_column }}, 1, 1) AS INT64) IN (7, 8, 9) THEN (
        SELECT
            MOD(
                d1 * 15 + d2 * 14 + d3 * 13 + d4 * 12 + d5 * 11 + d6 * 10
                + d7 * 9 + d8 * 8 + d9 * 7 + d10 * 6 + d11 * 5 + d12 * 4
                + d13 * 3 + d14 * 2 + d15 * 1,
                11
            ) = 0
        FROM (
