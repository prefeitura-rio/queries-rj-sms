...
CASE 
    ...
    ELSE (
        SELECT
            CASE
                WHEN calculated_first_digit = d0
                AND calculated_second_digit = d_unit
                    THEN TRUE
                ELSE FALSE
            END
        FROM (
            ...
            FROM (
                SELECT
                    *,
                    MOD(
                        d9*11 + d8*10 + d7*9 + d6*8 + d5*7 + d4*6 + d3*5 + d2*4 + d1*3 + calculated_first_digit*2,
                        11
                    ) AS second_sum_mod
                FROM (
                    SELECT
                    ...
                    FROM (
                        SELECT
                            *,
                            MOD(
                                d9*10 + d8*9 + d7*8 + d6*7 + d5*6 + d4*5 + d3*4 + d2*3 + d1*2,
                                11
                            ) AS first_sum_mod
                        FROM (
                            SELECT
