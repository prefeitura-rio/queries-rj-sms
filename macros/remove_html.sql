{% macro remove_html(text) %}
    ...
    -- Tags que precisam de substituições específicas + tags só removidas
    {% set tags = {
        '<br\\s*/?>':     '\\n',
        '</h[1-6]>':      '\\n',
        '<li\\b[^>]*>':   '- ',
        ...
        '</?ul\\b[^>]*>': '\\n',
        '</?ol\\b[^>]*>': '\\n',
        '</?(a|b|p|u|h[1-6]|em|div|img|span|strong|table|tbody|tr|td)\\b[^>]*>': '',
    } %}
    {% for tag, value in tags.items() %}
        {% set ns.sql_expr = "regexp_replace(" ~ ns.sql_expr ~ ", r'" ~ tag ~ "', '" ~ value ~ "')" %}
    {% endfor %}
