{% macro aux_remove_person_description(text) %}
    ...
    REGEXP_REPLACE(
        TRIM({{ text }}),
        r'(?i)^(HOME(M|N)?|MULHER)\b(\s|(PAR(D|T)|NEGR|BRANC|MOREN|PRET|JOVEM|ADULT|A(L|U)T|MEDIAN|MEDI|BAIX|MAGR|GORD|BARBUD|ALEGAD|DESCONHECID|BALEAD)(O|A)?|TRANS|MORADORA?\s*DE?\s*RUA|HOJE|ESTATURA|SEM NOME|UM|DOIS|APARENTA(NDO)?(\s*TER)?|MAIS|\+|OU|\/|\\|MENOS|\-|(DEZ[A-Z]*|(ON|DOU?|TRE|(CA|QUA)TOR|QUIN)ZE|VINTE|(TR?I|QUARE|CINQUE|SESS?E|SETE|OITE|NOVE)NTA)?(\s*E\s+[A-Z]+)?\s*(ANOS?|DE?\s*[A-Z]+\b)|(N(Ã|A)O|SEM)\s*ID[^\s]+|DECLAR(ADO|OU|A)(\s*SER)?|(IDENTIFICAD|CONHECID)(O|A)?\s*COMO|[\!-\@´`~\^\[\]_\{\}\|])*',
        ''
    )
{% endmacro %}
