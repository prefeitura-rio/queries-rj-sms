{{
    config(
        alias="vinculo_empregaticio",
    )
}}

with source as (select * from {{ source("brutos_cnes_web_staging", "tbModVinculo") }})
select
    -- Primary key
    safe_cast(cd_vinculacao as string) as codigo_vinculacao,
    safe_cast(ds_vinculacao as string) as descricao_vinculacao,
