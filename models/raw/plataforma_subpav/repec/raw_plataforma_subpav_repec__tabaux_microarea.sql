...
with
    source as (
        select *
        from {{ source("brutos_plataforma_subpav_staging", "subpav_repec__tabaux_microarea") }}
    ),

    tratar_campos as (
        select
            {{ process_null('codigoequipe') }} as codigoequipe,
            {{ process_null('codigomicroarea') }} as codigomicroarea,
