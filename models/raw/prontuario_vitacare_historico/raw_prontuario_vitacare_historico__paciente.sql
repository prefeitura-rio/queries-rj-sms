...
WITH
    ...
    fato_pacientes AS (
        SELECT
            ...
            SAFE_CAST({{ remove_double_quotes('frequentaescola') }} AS BOOLEAN) AS frequentaescola,
            {{ remove_double_quotes('nomemae') }} AS nomemae,
            {{ remove_double_quotes('nomepai') }} AS nomepai,
            SAFE_CAST({{ remove_double_quotes('membrocomunidadetradicional') }} AS BOOLEAN) AS membrocomunidadetradicional,
            ...
            SAFE_CAST({{ remove_double_quotes('participagrupocomunitario') }} AS BOOLEAN) AS participagrupocomunitario,
            SAFE_CAST({{ remove_double_quotes('possuiplanosaude') }} AS BOOLEAN) AS possuiplanosaude,
            {{ remove_double_quotes('relacaoresponsavelfamiliar') }} AS relacaoresponsavelfamiliar,
            SAFE_CAST({{ remove_double_quotes('territoriosocial') }} AS BOOLEAN) AS territoriosocial,
            {{ remove_double_quotes('escolaridade') }} AS escolaridade,
            {{ remove_double_quotes('identidadegenero') }} AS identidadegenero,
            SAFE_CAST({{ remove_double_quotes('criancamatriculadacrechepreescola') }} AS BOOLEAN) AS criancamatriculadacrechepreescola,
            SAFE_CAST({{ remove_double_quotes('emsituacaoderua') }} AS BOOLEAN) AS emsituacaoderua,
