...
WITH
    ...
    fato_tuberculose AS (
        SELECT
            ...
            safe_cast({{ process_null('datainiciotrat') }} as DATETIME) AS data_inicio_tratamento,
            safe_cast({{ process_null('idadeiniciotrat') }} as INT) AS idade_inicio_tratamento,
            safe_cast({{ process_null('pesoiniciotrat') }} as NUMERIC) AS peso_inicio_tratamento,
            ...
            safe_cast({{ process_null('numsinan') }} as NUMERIC) AS numero_sinan,
            {{ process_null('hiv') }} AS hiv,
            {{ process_null('comunicatb') }} AS comunica_tb,
            {{ process_null('raioxtorax') }} AS raio_x_torax,
            safe_cast({{ process_null('datainicioregistro') }} as DATETIME) AS data_inicio_registro,
            {{ process_null('resultadoraioxtorax') }} AS resultado_raio_x,
            ...
            {{ process_null('raioxtoraxmensal') }} AS raio_x_torax_mensal,
            {{ process_null('resultadoraioxtoraxacompanhamentomensal') }} AS resultado_raio_x_torax_acompanhamento_mensal,
            safe_cast({{ process_null('idadeexcltrat') }} as INT) AS idade_exclusao_tratamento,
            ...
            {{ process_null('raioxtoraxencerramento') }} AS raio_x_torax_encerramento,
            {{ process_null('resultadoraioxtoraxencerramento') }} AS resultado_raio_x_torax_encerramento,
            safe_cast({{ process_null('datainiciotratamentolatente') }} as DATETIME) AS data_inicio_tratamento_latente,
            {{ process_null('raioxtoraxtratamentolatente') }} AS raio_x_torax_tratamento_latente,
            ...
            {{ process_null('resultadoraioxtoraxtratamentolatente') }} AS resultado_raio_x_torax_tratamento_latente,
            {{ process_null('esquemailtb') }} AS esquema_ilbt,
            {{ process_null('observacoestuber') }} AS observacoes_tuberculose,
