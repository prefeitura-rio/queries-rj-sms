{{
    config(
        alias="receituario_especializado",
        materialized="table",
...
with source as (
    ...
    receituario_filtrado as (
        ...
        where json_value(doc, '$.tipo') = 'RECEITUÁRIO ESPECIALIZADO (LME)'
    ),
    ...
    receituario as (
        select
            ...
            {{ base64_to_string("json_extract_scalar(doc_receituario, '$.dados.anamnese')") }} as anamnese,

            json_extract_scalar(doc_receituario, '$.dados.peso') as peso,
            json_extract_scalar(doc_receituario, '$.dados.altura') as altura,
            json_extract_scalar(doc_receituario, '$.dados.tratamento') as flag_tratamento,
            json_extract_scalar(doc_receituario, '$.dados.incapaz') as flag_incapaz,
            {{ base64_to_string("json_extract_scalar(doc_receituario, '$.dados.relato_tratamento')") }} as relato_tratamento,

            json_extract_scalar(doc_receituario, '$.dados.responsavel') as responsavel,
            json_extract_scalar(doc_receituario, '$.dados.cid') as cid,
    ...
    cast_rename as (
        select
            ...
            safe_cast({{ process_null('anamnese') }} as string) as anamnese,

            safe_cast({{ process_null('peso') }} as float64) as peso,
            safe_cast({{ process_null('altura') }} as int64) as altura,
            safe_cast({{ process_null('flag_tratamento') }} as boolean) as flag_tratamento,
            safe_cast({{ process_null('flag_incapaz') }} as boolean) as flag_incapaz,
            safe_cast({{ process_null('relato_tratamento') }} as string) as relato_tratamento,
            safe_cast({{ process_null('responsavel') }} as string) as responsavel,
            safe_cast({{ process_null('cid') }} as string) as cid,
