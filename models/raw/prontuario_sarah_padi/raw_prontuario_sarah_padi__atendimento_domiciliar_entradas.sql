...
with source as (
    select
        *
    from {{ source("brutos_prontuario_sarah_padi_staging", "atendimento_domiciliar_entradas") }}
    {% if is_incremental() %}
...
base64_para_string as (
    select
        ...
        {{ base64_to_string('motivo_alta') }} as motivo_alta,
        {{ base64_to_string('motivo_alta_id') }} as motivo_alta_id,
...
renomeado as (
    select 
        ...
        safe_cast(paciente_genero_id as int64) as paciente_id_genero,
        ...
        safe_cast(tipo_origem as string) as tipo_origem,
