...
with
    source as (
        select 
            * 
        from {{ source("brutos_exames_laboratoriais_staging", "solicitacoes") }}
    ),
    ...
    renamed as (	
        select
            ...
            safe_cast({{process_null("unidade")}} as string) as area_programatica,
            safe_cast({{process_null("origem")}} as string) as unidade,
            safe_cast({{process_null("codunidade")}} as string) as codigo_unidade,
            safe_cast({{process_null("codorigem")}} as string) as codigo_origem,

            safe_cast({{parse_datetime(process_null("dataPedido"))}} as datetime) as datahora_pedido,
            ...
            safe_cast({{process_null("paciente_codigoLis")}} as string) as paciente_codigo_lis,
            safe_cast({{process_null("paciente_nome")}} as string) as paciente_nome,
            safe_cast({{parse_date(process_null("paciente_nascimento"))}} as date) as paciente_data_nascimento,
            CASE 
