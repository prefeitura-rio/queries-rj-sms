...
{% set partitions_to_replace = "date_sub(current_date('America/Sao_Paulo'), interval 30 day)" %}
...
with base as (
    ...
    {% if is_incremental() %}
      where date(safe_cast(json_extract_scalar(data, '$.datahora_fim_atendimento') as datetime)) >= {{ partitions_to_replace }}
    {% endif %}
),
