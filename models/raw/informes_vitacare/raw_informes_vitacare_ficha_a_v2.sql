...
with
    ...
    extrair_informacoes as (
        select
            ...
            safe_cast({{ process_null('data_cadastro') }} as date) as data_cadastro,
            safe_cast({{ process_null('data_ultima_atualizacao_do_cadastro') }} as date) as data_ultima_atualizacao_cadastro,
            ...
            safe_cast({{ process_null('data_de_nascimento') }} as date) as data_nascimento,
            ...
            safe_cast({{ process_null('data_1a_consulta') }} as date) as data_1a_consulta,
            safe_cast({{ process_null('data_2a_consulta') }} as date) as data_2a_consulta,
            safe_cast({{ process_null('data_da_ultima_consulta') }} as date) as data_ultima_consulta,
            safe_cast({{ process_null('data_da_ultima_consulta_med_enf_propria_equipe') }} as date) as data_ultima_consulta_med_enf_propria_equipe,
            {{ process_null('hist_cid') }} as hist_cid,
            -- CIDs e datas de registro
            {% for i in range(1, 11) %}
                {% set idx = "%02d" % i %}
                    safe_cast({{ process_null('data_registro_cid_' ~ idx) }} as date) as data_registro_cid_{{ idx }},
                    {{ process_null('cid_' ~ idx) }} as cid_{{ idx }}{% if not loop.last %},{% endif %}
            {% endfor %},
            {{ process_null('peso') }} as peso,
            {{ process_null('altura') }} as altura,
            safe_cast({{ process_null('data_registro_peso_altura') }} as date) as data_registro_peso_altura,
            safe_cast({{ process_null('data_ultima_menstruacao') }} as date) as data_ultima_menstruacao,
