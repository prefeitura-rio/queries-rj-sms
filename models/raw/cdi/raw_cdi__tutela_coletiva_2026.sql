{{
    config(
        schema = "brutos_cdi",
        alias = "tutela_coletiva_2026",
        materialized = "table"
    )
}}

select
    {{ normalize_null("regexp_replace(regexp_replace(trim(sei), r'(?i)^(#ref!|#value!)$', ''), r'\\s+', ' ')") }} as processo_rio,
    ...
    {{ cdi_parse_date('data_do_of', 'sei', 'oficio') }} as data_of,
    {{ cdi_parse_date('data_da_entrada', 'sei', 'oficio') }} as data_entrada,
    safe_cast({{ normalize_null('prazo__dias') }} as int64) as prazo_dias,
    {{ cdi_parse_date('vencimento', 'sei', 'oficio') }} as vencimento,
    {{ cdi_parse_date('data_de_saida', 'sei', 'oficio') }} as data_saida,
    {{ cdi_parse_date('data_do_envio_ao_orgao_solicitante__arquivamento', 'sei', 'oficio') }} as data_envio_orgao_solicitante_arquivamento,
    ...
    {{ normalize_null("regexp_replace(regexp_replace(trim(sei_status), r'(?i)^(#ref!|#value!)$', ''), r'\\s+', ' ')") }} as sei_status,
