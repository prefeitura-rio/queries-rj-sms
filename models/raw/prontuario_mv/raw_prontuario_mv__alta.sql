...
alta_json as (
    select 
        ...
        json_extract_scalar(data, '$.procedimentos_realizados') as procedimentos_realizados,
        json_extract_scalar(data, '$.resultados_obtidos') as resultados_obtidos,
        json_extract_scalar(data, '$.medicamentos_utilizados') as medicamentos_utilizados,
        json_extract_scalar(data, '$.evolucao_paciente') as evolucao_paciente,
        json_extract_scalar(data, '$.plano_alta_orientacao_nutricionista') as plano_alta_orientacao_nutricionista,
        json_extract_scalar(data, '$.plano_alta_orientacao_enfermagem') as plano_alta_orientacao_enfermagem,
        json_extract_scalar(data, '$.plano_alta_orientacao_fisioterapia') as plano_alta_orientacao_fisioterapia,
        json_extract_scalar(data, '$.plano_alta_orientacao_psicologia') as plano_alta_orientacao_psicologia,
        json_extract_scalar(data, '$.orientacao_medica_alta') as orientacao_medica_alta,
        json_extract_scalar(data, '$.local_transferencia') as local_transferencia,
        json_extract_scalar(data, '$.data_hora_alta_medica') as data_hora_alta_medica,
        ...
        json_extract_scalar(data, '$.cid_principal_inativo') as cid_principal_inativo,
...
alta_renomeado as (
    select
        ...
        -- Alta
        safe.parse_datetime('%Y/%m/%d %H:%M:%S', data_hora_alta_medica) as alta_medica_datahora,
        safe.parse_datetime('%Y/%m/%d %H:%M:%S', dh_fechamento) as alta_datahora_fechamento,
        safe_cast({{ process_null('dias_internado') }} as int64) as dias_internado,
        {{ process_null('local_transferencia') }} as local_transferencia,

        -- Plano de Alta
        {{ process_null('plano_alta_orientacao_nutricionista') }} as plano_alta_orientacao_nutricionista,
        {{ process_null('plano_alta_orientacao_enfermagem') }} as plano_alta_orientacao_enfermagem,
        {{ process_null('plano_alta_orientacao_fisioterapia') }} as plano_alta_orientacao_fisioterapia,
        {{ process_null('plano_alta_orientacao_psicologia') }} as plano_alta_orientacao_psicologia,
