...
parecer_json as (
    select 
        ...
        json_extract_scalar(data, '$.data_solicitacao_parecer') as data_solicitacao_parecer,
        json_extract_scalar(data, '$.prestador_solicitante_nome') as prestador_solicitante_nome,
        json_extract_scalar(data, '$.especialidade_requisitada') as especialidade_requisitada,
        ...
        json_extract_scalar(data, '$.justificativa_solicitacao') as justificativa_solicitacao,
...
parecer_renomeado as (
    select
        ...
        -- Parecer
        safe.parse_datetime('%Y/%m/%d %H:%M:%S', data_solicitacao_parecer) as solicitacao_parecer_datahora,
        {{ process_null('prestador_solicitante_nome') }} as solicitante_nome,
        {{ process_null('especialidade_requisitada') }} as especialidade_requisitada,
        {{ process_null('prestador_solicitado') }} as solicitado_nome,
        {{ process_null('situacao_parecer') }} as parecer_situacao,
        {{ process_null('justificativa_solicitacao') }} as solicitacao_justificativa,
