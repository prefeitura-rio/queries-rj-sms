...
with
    ...
    bam_json as (
        select
            ...
            json_extract_scalar(
                data, '$.medicamento_uso_continio'
            ) as medicamento_uso_continio,
            json_extract_scalar(data, '$.exame_fisico') as exame_fisico,
            json_extract_scalar(
                data, '$.exames_complementares'
            ) as exames_complementares,
            ...
            json_extract_scalar(
                data, '$.destino_pos_atendimento'
            ) as destino_pos_atendimento,
            json_extract_scalar(
            ...
            json_extract_scalar(
                data, '$.ds_historico_doenca_atual'
            ) as ds_historico_doenca_atual,
            json_extract_scalar(data, '$.ds_queixa_medica') as ds_queixa_medica,
            json_extract_scalar(
                data, '$.ds_historico_doenca_pregressa'
            ) as ds_historico_doenca_pregressa,
            json_extract_scalar(data, '$.ds_historico_social') as ds_historico_social,
            json_extract_scalar(
                data, '$.ds_exame_fisico_especifico'
            ) as ds_exame_fisico_especifico,
    ...
    bam_renomeado as (
        select
            ...
            {{ process_null("ds_historico_doenca_pregressa") }}
            as historia_doenca_pregressa,
            ...
            safe_cast(
                {{ process_null("sinal_vital_pas") }} as float64
            ) as pressao_arterial_sistolica,
            safe_cast(
                {{ process_null("sinal_vital_pad") }} as float64
            ) as pressao_arterial_diastolica,
            safe_cast(
                {{ process_null("sinal_vital_fc") }} as float64
            ) as frequencia_cardiaca,
            safe_cast(
                {{ process_null("sinal_vital_fr") }} as float64
            ) as frequencia_respiratoria,
            safe_cast({{ process_null("sinal_vital_tax") }} as float64) as temperatura,
            safe_cast(
                {{ process_null("sinal_vital_spo2") }} as float64
