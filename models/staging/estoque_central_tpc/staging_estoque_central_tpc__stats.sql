...
with
    ...
    -----------------------------------------
    entidades as (
        select tipo from unnest(['stock-position']) tipo
    ),
    datas as (
    ...
    unidades as (
        select 'central' as estabelecimento_cnes
    ),
    todos_registros_possiveis as (
        select 
            cast(unidades.estabelecimento_cnes as string) as estabelecimento_cnes,
    ...
    estoque_posicao as (
        select 
            'central' as estabelecimento_cnes,
