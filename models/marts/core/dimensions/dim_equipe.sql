...
contato_equipe as (
    ...
    safe_cast(updated_at as timestamp) as updated_at_telefone
    ...
    equipe_completa as (
        select
            ...
            greatest(
                safe_cast(equipe.data_atualizacao as timestamp),
                safe_cast(profissionais_equipe.ultima_atualizacao_profissionais as timestamp)
            ) as updated_at_rank,
            contato_equipe.updated_at_telefone
        from equipe
        inner join dim_estabelecimentos 
            on dim_estabelecimentos.id_unidade = equipe.id_unidade
        left join profissionais_equipe
            on profissionais_equipe.id_unidade_saude = equipe.id_unidade
           and profissionais_equipe.equipe_sequencial = equipe.equipe_sequencial
        left join dim_area 
            on dim_area.id_area = equipe.id_area
        left join dim_tipo_equipe 
            on dim_tipo_equipe.id_equipe_tipo = equipe.id_tipo_equipe
        left join contato_equipe 
            on lpad(contato_equipe.ine, 10, '0') = equipe.equipe_ine
    )

select *
except(updated_at_rank, updated_at_telefone)
from equipe_completa
qualify row_number() over (
    partition by id_ine
    order by updated_at_rank desc, updated_at_telefone desc
) = 1
