{{
    config(
        ...
        alias="alocacao_profissional_serie_historica",
        materialized="table",
    )
}}
...
SELECT
  ...
  p.tipo_conselho,
  cod_sus.CO_PROFISSIONAL_SUS as cod_profissional_sus,
...
LEFT JOIN (
  SELECT *
  FROM `rj-sms-dev.brutos_cnes_web_staging.tbDadosProfissionalSus`
  WHERE _data_snapshot = (
    SELECT DISTINCT _data_snapshot 
    FROM `rj-sms-dev.brutos_cnes_web_staging.tbDadosProfissionalSus`
    ORDER BY 1 DESC 
...
ON cartao_nacional_saude = cod_sus.CO_CNS
