...
with
    ...
    pacientes_base as (
        select distinct
            id_hash as id_base,
            cast(null as string) as cpf_base, -- Não tem CPF no SIM
            {{ clean_name_string("upper(nome)") }} as nome_base,
            {{ clean_name_string("upper(nome_mae)") }} as nome_mae_base,
            cast(data_nascimento as date) as data_nascimento_base,
