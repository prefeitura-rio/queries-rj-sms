...
dispensacao as (
    select 
        ...
        iqr,
        outlier
    from {{ ref('int_estoque__dispensacao_serie_historica_com_outliers_identificados') }} where row_num <= 90),

estabelecimento as (select * from {{ ref('dim_estabelecimento') }}),

material_dimensao as (select * from {{ ref('dim_material') }}),

material_alternativo as (select distinct id_cnes, id_material, material_descricao from {{ ref('fct_estoque_posicao') }})
