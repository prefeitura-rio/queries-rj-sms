...
cad_e_atd AS (
  ...
  linha_cadastro AS (
    ...
    LEFT JOIN {{ ref('raw_prontuario_vitacare_historico__cadastro') }} cad
      ON lt.cpf = cad.cpf
  ),
  ...
  atendimentos_por_unidade AS (
    ...
    JOIN {{ ref('mart_bi_gestacoes__atendimentos_prenatal_aps') }} apa
...
condicoes_flags AS (
    SELECT
        ...
        MAX(
            CASE
                WHEN REGEXP_CONTAINS(cg.cid, r'\bA5[1-3]\b')
...
--====================================================*************************************************************
-- CORRIGIR PARA ENGLOBAR ENCAMINHAMENTOS SISREG E SER
--====================================================*************************************************************
...
prenatal_risco_marcadores AS (
  ...
  FROM filtrado f
  JOIN pacientes_info pi ON f.id_paciente = pi.id_paciente
  LEFT JOIN {{ ref('raw_prontuario_vitacare_historico__acto') }} a
    ON pi.cpf = a.patient_cpf
  LEFT JOIN {{ ref('raw_prontuario_vitacare_historico__prenatal') }} pn
...
incluir_AP AS (
    ...
    FROM
        ...
        -- LEFT JOIN {{ ref('mart_historico_clinico__paciente') }} paciente 
        LEFT JOIN {{ ref('mart_historico_clinico__paciente') }} paciente
...
Urgencia_e_emergencia as (
    ...
    FROM ( -- Adicionado subquery para aplicar ROW_NUMBER e filtrar
            ...
            FROM
                ...
                -- LEFT JOIN {{ ref('mart_bi_gestacoes__consultas_emergenciais') }} fapn 
                LEFT JOIN {{ ref('mart_bi_gestacoes__consultas_emergenciais') }} fapn
                    ON f.id_gestacao = fapn.id_gestacao
...
final AS (
    ...
    SELECT
        ...
        CASE
            WHEN f.fase_atual IN ('Gestação') THEN DIV(DATE_DIFF (
                CURRENT_DATE(),
                f.data_inicio,
                DAY
            ),7)
        ...
        CASE
            ...
            AND f.data_fim_efetiva IS NOT NULL THEN DIV(DATE_DIFF (
                f.data_fim_efetiva,
                f.data_inicio,
                DAY
            ),7)
    ...
    -- Encaminhamento HAS
    -- COALESCE(hgc.tem_encaminhamento_has, 0) as tem_encaminhamento_has,
    ...
    frpa.total_fatores_alto_risco_aas,
    frpa.total_fatores_moderado_aas,
    frpa.imc_inicio,
    frpa.tem_obesidade,
    ...
    CASE
        ...
        ELSE NULL
    END AS IG_atual_semanas_aas,
    hgc.tem_prescricao_aas as tem_prescricao_aas2,
