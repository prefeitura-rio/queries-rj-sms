...
paciente_identificadores AS (
    ...
    -- JOIN rj-sms.saude_historico_clinico.paciente p 
    JOIN {{ ref('raw_saude_historico_clinico__paciente') }} p 
    ON g.id_paciente = p.dados.id_paciente
...
sisreg_pre_filtrado AS (
    SELECT
        pi.id_paciente,
        s.*
    FROM {{ ref('raw_brutos_sisreg_api__solicitacoes') }} s
