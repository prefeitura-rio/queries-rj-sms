{{
    config(
        alias='serie_temporal_ingestao_unidade_vitai',
...
staging as (
  {% for item in tabelas %}
    select 
      gid,
      '{{ item.nome }}' as tabela,
      {% if item.nome == 'atendimento'%}
        estabelecimento_id as estabelecimento_gid,
      {% else %}
        estabelecimento_gid,
      {% endif %}
...
ingestao as (
    ...
    from staging
    group by tabela, data_envio, estabelecimento_gid
),
...
final as (
    ...
    from ingestao i
    left join estabelecimento e on e.gid = i.estabelecimento_gid
)
